/*
      # Fix Profile Permissions, Defaults, and RLS (v3 - EXCLUDED syntax)
      ...
      2. Updating the `handle_new_user` trigger to populate these fields, using EXCLUDED syntax in ON CONFLICT for clarity.
      ...
      This aims to resolve "permission denied" errors and parsing errors with "NEW" in trigger functions.
      ...
      2. Trigger Modifications (`public.handle_new_user`):
         ...
         - Uses `EXCLUDED.column` and `profiles.column` in the `ON CONFLICT DO UPDATE` clause for better clarity and to avoid parser issues with `NEW`/`OLD` in this context.
    ...
    */
    ...
    BEGIN
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        -- If existing profile had NULL role/status, update with proposed default. Otherwise, keep existing.
    ...
    WITH CHECK (
      auth.uid() = id AND
      (NEW.role = OLD.role OR OLD.role IS NULL) AND 
      (NEW.status = OLD.status OR OLD.status IS NULL)
    );
