/*
      # Fix Profile Permissions, Defaults, RLS (v5 - Trigger Variables)

      This migration attempts to resolve the "missing FROM-clause entry for table new" error
      by assigning NEW.id and NEW.raw_user_meta_data values to local variables within
      the `handle_new_user` trigger function before using them in the INSERT statement.
      ...
      2. Trigger Modifications (`public.handle_new_user`):
         - **Assigns `NEW.id` and relevant parts of `NEW.raw_user_meta_data` to local variables.**
         - Uses these local variables in the `VALUES` clause of the `INSERT`.
         - The `ON CONFLICT` clause remains, using `EXCLUDED` and `public.profiles` references.
         - Returns `NULL` as it's an `AFTER` trigger.
    ...
    */
    ...
    RAISE NOTICE 'Applied RLS policies to public.profiles. Attempted fix with trigger variables.';
