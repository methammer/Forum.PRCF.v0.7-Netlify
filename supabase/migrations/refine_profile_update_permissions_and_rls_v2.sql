/*
  # Refine Profile Update Permissions and RLS (v2)

  This migration ensures that the database schema correctly handles profile updates,
  specifically concerning role and status modifications. This version is to re-affirm
  the correct state after potential issues or to ensure it's applied correctly.
...
/*
  ...
  - After this migration, the `update-user-details-admin` Edge Function (using service_role_key)
    should be able to successfully update user roles and statuses (assuming roles are sent in lowercase
    if the DB expects lowercase).
  - Authenticated users attempting to update their own `role` or `status` via a direct SQL
    update should be blocked by the updated RLS policy's `WITH CHECK` clause.
